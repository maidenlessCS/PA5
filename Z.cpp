...
void Z::rotate(char board[20][10]) {
...
char Z::getChar() { return mChar; }

float Z::getCenterX() { return centerX; }
float Z::getCenterY() { return centerY; }

int Z::getStartingRow() { return startRow; }

vector<vector<char>> Z::getSprite() {
...
bool Z::movePos(char board[20][10], int moveX, int moveY) {
    // COLLISION CHECKING
    for(int i = -2; i <= 2; i++) {                     
        for(int j = -2; j <= 2; j++) {                  
...
bool Z::canMove(char board[20][10], int moveX, int moveY, int i, int j) {
