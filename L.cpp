...
void L::rotate(char board[20][10]) {
    ...
    do {
        ...
        for(int i = -2; i <= 2; i++) {                     
            for(int j = -2; j <= 2; j++) {                  
                if((shape[index][i+2][j+2] == mChar) && (posX+j >= 10 || posX+j < 0 || board[posY+i][posX+j] != '-' || ((index == 0 || index == 2) && posX >= 7))) {
                        canRotate = false;
                        break;
                    }
            }                                               
...
char L::getChar() { return mChar; }

float L::getCenterX() { return centerX; }
float L::getCenterY() { return centerY; }

int L::getStartingRow() { return startRow; }

vector<vector<char>> L::getSprite() {
...
bool L::movePos(char board[20][10], int moveX, int moveY) {
    // COLLISION CHECKING
    for(int i = -2; i <= 2; i++) {                     
        for(int j = -2; j <= 2; j++) {                  
...
bool L::canMove(char board[20][10], int moveX, int moveY, int i, int j) {
