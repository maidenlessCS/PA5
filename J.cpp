...
void J::rotate(char board[20][10]) {
...
char J::getChar() { return mChar; }

float J::getCenterX() { return centerX; }
float J::getCenterY() { return centerY; }

int J::getStartingRow() { return startRow; }

vector<vector<char>> J::getSprite() {
...
bool J::movePos(char board[20][10], int moveX, int moveY) {
    // COLLISION CHECKING
    for(int i = -2; i <= 2; i++) {                     
        for(int j = -2; j <= 2; j++) {                  
...
bool J::canMove(char board[20][10], int moveX, int moveY, int i, int j) {
