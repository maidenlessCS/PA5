...
void O::rotate(char board[20][10]) {
...
char O::getChar() { return mChar; }

float O::getCenterX() { return centerX; }
float O::getCenterY() { return centerY; }

int O::getStartingRow() { return startRow; }

vector<vector<char>> O::getSprite() {
...
bool O::movePos(char board[20][10], int moveX, int moveY) {
    // COLLISION CHECKING
    for(int i = -2; i <= 2; i++) {                     
        for(int j = -2; j <= 2; j++) {                  
...
bool O::canMove(char board[20][10], int moveX, int moveY, int i, int j) {
